...
TEST_CASE("conversion to Scalars") {
	...
	SECTION("same iteration order") {
		...
		SECTION("order jki") {
			...
			std::transform(scal_node.begin(), scal_node.end(), std::back_inserter(observed), [&](auto&& e) { return std::make_pair(e.first, e.second); });
			REQUIRE(observed.size() == 3);
			CHECK(observed[0].first.s == "j");
			CHECK(observed[0].second.get<int>() == 2);
			CHECK(observed[1].first.s == "k");
			CHECK(observed[1].second.get<int>() == 3);
			CHECK(observed[2].first.s == "i");
			CHECK(observed[2].second.get<int>() == 1);
		}
	}
}
